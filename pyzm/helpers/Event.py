...
class Event:
    ...
    def get_image_url(self, fid="snapshot"):
        ...
        """
        eid = self.id()
        url = f"{g.api.portal_url}/index.php?view=image&eid={eid}&fid={fid}&{g.api.get_auth()}"
        return url
    ...
    def get_video_url(self) -> Optional[AnyStr]:
        ...
        if not self.video_file():
            g.logger.error(
                f"Event '{self.id()}' does not have a video file, do you have 'Video Writer' set to "
                f"'Encode' or 'Camera Passthrough'?"
            )
            return None
        eid = self.id()
        url = g.api.portal_url + "/index.php?mode=mpeg&eid={}&view=view_video".format(eid) + "&" + g.api.get_auth()
        return url
    ...
    def delete(self):
        ...
        Returns:
            json: API response
        """
        url = f"{g.api.api_url}/events/{self.id()}.json"
    ...
    def _download_file(self, url, file_name, dest_dir, show_progress=False, reauth=True):
        ...
        try:
            ...
            if g.api and g.api.get_session():
                req = g.api.get_session()
                # print ("OVERRIDING")
        ...
        if r.headers.get("Content-Type") and "text/html" in r.headers.get("Content-Type"):
            if reauth:
                g.logger.error(
                    "It seems we were redirected to login page while trying to download, trying one more time"
                )
                return self._download_file(url, file_name, dest_dir, show_progress, reauth=False)
            else:
