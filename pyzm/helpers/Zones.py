...
class Zones:
    def __init__(self, mid: Optional[Union[int, str]] = None):
        global g
        g = GlobalConfig()
    ...
    def _load(self, options: Optional[dict] = None):
        if options is None:
            options = {}
        g.logger.info("Retrieving zones via API")
    ...
    def add(self, options: Optional[dict] = None) -> dict:
        ...
        if payload and len(payload):
            if (
                payload.get("Zone[MonitorId]") and payload.get("Zone[Type]") and payload.get("Zone[Name]")
            ):  # need at least these 3 things minimum
