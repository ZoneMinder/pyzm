...
class MediaStream:
    ...
    def __init__(self, stream=None, type_: Optional[str] = None, options=None):
        if type_ is None:
            type_ = "video"
        if not options:
        ...
        if (isinstance(self.stream, str) and self.stream.isnumeric()) or (isinstance(self.stream, int)):
            ...
            if self.options.get("pre_download"):
                ...
                g.logger.debug(f"{lp} Download event images locally before running detections for stream: {g.eid} ")
                api_events = self.api.events({"event_id": int(g.eid)})

                if not api_events:
                    g.logger.error(f"{lp} no such event {g.eid} found with API")
                    return
                # should only be 1 event as we sent a filter for that event in the call
                for event in api_events:
                    self.stream = event.Event.download_video(download_dir=self.options.get("pre_download_dir", "/tmp/"))
            ...
            # use API
                ...
                g.logger.debug(2, f"{lp} setting 'image' as type for event -> '{g.eid}'")
                self.type = "image"
        ...
        if self.options.get("frame_set"):  # process frame_set
            ...
            if self.type == "video" and ("alarm" in self.frame_set or "snapshot" in self.frame_set):
                ...
                self.frame_set = [str(i) for i in self.options.get("frame_set") if i != "snapshot" or i != "alarm"]
        ...
        if self.type == "video":
            ...
            g.logger.debug(f"{lp}VID: first load - skipping to frame {self.start_frame}")
            if self.frame_set:
                while self.fvs.more() and self.frames_read <= self.frame_set_len:
                    self.fvs.read()
                    self.frames_read += 1
            else:
                while self.fvs.more() and self.next_frame_id_to_read < int(self.start_frame):
                    self.fvs.read()
    ...
    def read(self):
            ...
            while True:
                ...
                if frame is None:
                    ...
                    if self.frames_before_error >= self.contig_frames_before_error:
                        g.logger.error(f"{lp} Error reading frame #-{self.frames_read}")
                        return
                    else:
                        g.logger.debug(
                ...
                self.frames_before_error = 0
                self.orig_h_w = frame.shape[:2]
                if self.frame_set and (self.frames_read != int(self.frame_set[self.frame_set_index])):
                    continue
                ...
                if self.frame_set:
                    ...
                    if (self.last_frame_id_read - 1) % self.frame_skip:
                        g.logger.debug(5, f"{lp} skipping frame {self.last_frame_id_read}")
                        continue
                ...
                self.fids_processed.append(self.last_frame_id_read if self.last_frame_id_read != 0 else None)
                self.frames_processed += 1
                break
            # END OF WHILE LOOP

            g.logger.debug(f"{lp} ***** RESIZE={self.options.get('resize')}")
            if self.options.get("resize", "no") != "no":
        ...
        # grab images frame by frame from ZM API (can pre download frames and then process)
            ...
            if not g.api_event_response or not past_event or not g.Frame or not g.Event:
                try:
            ...
            try:
                ...
                g.logger.error(
                    f"ZeroDivisionError while attempting to figure out FPS, the frame buffer needs to "
                    f"grow before a detection."
                )
                f_fps = None
            ...
            if self.frames_processed > 0 or self.frames_skipped > 0 and f_fps is not None:
                ...
                if (oob_frame_id >= g.event_tot_frames) and not skip_all:
                    ...
                    if f_difference <= (f_fps * fps_thresh) and not past_event:
                        ...
                        self.last_frame_id_read = comp_fid = self.frame_set[self.frame_set_index] = str(
                            g.event_tot_frames - 2
                        )
                elif oob_frame_id >= g.event_tot_frames and skip_all:
                    g.logger.debug(
                ...
                for x in range(max_attempts):
                    ...
                    try:
                        ...
                        if sleep_time and not past_event:
                            print(
                                f"BEFORE REQUEST: it is not a past event and {sleep_time=} -- requesting event "
                            ...
                            g.Event, g.Monitor, g.Frame = self.api.get_all_event_data()
                            print(
                                f"AFTER REQUEST: ZM Frame Buffer has {g.event_tot_frames} frames, we want this frame "
                                f"id -> {comp_fid}"
                            )
                            sleep(sleep_time)
                    ...
                    try:
                        ...
                        if str2bool(self.options.get("save_frames")):
                            ...
                            if Path(d).is_dir():
                                ...
                                g.logger.error(
                                    f"{lp} there is aproblem with the configured 'save_frames_dir' "
