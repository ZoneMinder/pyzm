...
class AlprBase:
    def __init__(self, options: Optional[dict] = None, tempdir: str = "/tmp"):
        global g
        g = GlobalConfig()
        if options is None:
            options = {}
    ...
    def prepare(self, alpr_object):
        if not isinstance(alpr_object, str):
            g.logger.debug(f"{lp} the supplied object is not an absolute file path, assuming blob and creating file")
...
class PlateRecognizer(AlprBase):
    ...
    def __init__(self, options=None, tempdir="/tmp"):
        ...
        g.logger.debug(f"{lp}plate rec: initialized with url: {self.url}")
    ...
    def detect(self, input_image=None):
        ...
        with open(self.filename, "rb") as fp:
            try:
                ...
                g.logger.error(f"{lp}plate rec: API rejected the upload with {e} and body:{c}")
            else:
                response = response.json()
...
class OpenAlpr(AlprBase):
    def __init__(self, options=None, tempdir="/tmp"):
        ...
        g.logger.debug(f"{lp} OpenALPR initialized with url: {self.url}")
    ...
    def detect(self, input_image=None):
        ...
        with Path(self.filename).open("rb") as fp:
            try:
                ...
                g.logger.debug(f"Open ALPR rejected the upload with {e}")
            else:
                response = response.json()
